...
For iterative validation where the consumer has an explicit `.github/validation-impact.json`, `validation-evidence.yml` can reuse successful evidence across unrelated source changes. Its fingerprint includes the selected unit's dependency closure, declared global inputs, exact tracked input bytes, command/setup identity, runner identity, and an optional caller-owned environment identity. The source SHA is recorded in each receipt but deliberately stays outside the reusable fingerprint; otherwise unrelated commits could never reuse unchanged evidence. Any missing/invalid fingerprint, artifact lookup error, expired receipt, digest mismatch, or verification failure falls back to executing the validation command.
